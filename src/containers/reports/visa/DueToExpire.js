...
const DueToExpireV = () => {
  ...
  const generateReport = (elementId) => {
    ...
    html2pdf(element, {
      margin: 2,
      filename: `Visas due-${getReadableTimestamp()}.pdf`,
    ...
    axios
      ...
      .then((res) => {
        if (res.data.error) {
          ...
          setRecords(res.data.visas);

          setFlattenedData(prev => {
            return res.data.visas.map((item) => ({
                visaNo: item.visa.viId,
                idNo: item.visa.Employee.idNo,
                fullName: item.visa.Employee.firstName + " " + item.visa.Employee.lastName,
                nationality: item.visa.Employee.nationality,
                bloodGroup: item.visa.Employee.Health.bloodGroup,
                agent: item.visa.Employee.Agent.name,
                expiredOn: item.visa.expiryDate,
  ...
  return (
    <div className='w-full flex h-full flex-col items-center justify-between relative gap-3'>
      ...
      <div className='w-full flex justify-center mt-5 gap-2 p-2'>
...

        {
            ...
            <>
                ...
                <CSVLink
                data={flattenedData}
                filename={`visas due-`+ getReadableTimestamp()}
...

      {
        ...
        <h1 className='text-[14pt] self-center mt-2'>Report Listing- Visas Due to Expire</h1>
            ...
            <div className="overflow-x-auto">
                ...
                <tbody className="text-gray-600 text-sm font-light">
                    ...
                    <tr key={index} className="border-b border-gray-200 hover:bg-gray-100">
                        <td className="py-3 px-6 text-left whitespace-nowrap">{item.visaNo}</td>
                        ...
                        <td className="py-3 px-6 text-left">{Math.abs(parseInt(item.days)) + " days to come"}</td>
                    </tr>
                    ))}
                </tbody>
                </table>
